...
- **`export --format json` carries custom frontmatter** — each entry gains a `frontmatter` object with the page's remaining frontmatter fields (everything not already surfaced as a top-level export field), preserving YAML typing; omitted when a page has no extra fields. Custom fields declared by registered types (e.g. a `decision` type's `created`, `deciders`, `supersedes`) now survive the export seam without per-page `content read` fallbacks. `list` output is unchanged (#11)
